...
export async function cancelarReservaCliente({ inscripcionId, claseId, usuarioId }) {
  ...
  await runTransaction(db, async (transaction) => {
    ...
    if (inscripcionDoc.data().estado !== "confirmada") {
      throw new Error("La reserva ya ha sido cancelada previamente.");
    }

    // 2. Obtener la clase y verificar el plazo de 15 horas
    const claseDoc = await transaction.get(claseRef);
    if (!claseDoc.exists()) {
      throw new Error("La clase asociada a esta reserva ya no está disponible.");
    }

    const claseData = claseDoc.data();
    const fechaInicioVal = claseData.fecha_inicio;
    const fechaClaseObj = fechaInicioVal.toDate ? fechaInicioVal.toDate() : new Date(fechaInicioVal);
    const ahora = new Date();

    const diferenciaHoras = (fechaClaseObj.getTime() - ahora.getTime()) / (1000 * 60 * 60);

    if (diferenciaHoras < 15) {
      throw new Error("Solo puedes anular una reserva con al menos 15 horas de anticipación al inicio de la clase.");
    }

    // 3. Marcar la inscripción como cancelada
    transaction.update(inscripcionRef, {
      estado: "cancelada"
    });

    // 4. Liberar cupo en la clase
    const inscritosActuales = claseData.inscritos_ids || [];
    ...
    // 5. Reintegrar 1 clase pactada al usuario
    const usuarioDoc = await transaction.get(usuarioRef);
    let clasesActuales = 0;
    if (usuarioDoc.exists()) {
      clasesActuales = usuarioDoc.data().clases_pactadas ?? 0;
    }

    transaction.set(usuarioRef, { clases_pactadas: clasesActuales + 1 }, { merge: true });
  });

  return { success: true, message: "Reserva anulada con éxito y clase pactada reintegrada." };
}
